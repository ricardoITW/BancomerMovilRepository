...
public class Cambio_de_numero_tel_sucursal {
	...
	public static void Ejecutar() {
		...
		try{
			if(VentanaPrincipal.existeLblHola()){
				...
				Thread.sleep(1500);
				EvidenceData.capturarPaso(true, "Se muestra ventana ¡Bienvenido! a BBVA Bancomer", driver);
				...
				if(IngresarTarjeta.existeTxtNumTarjeta()){
					...
					if(IngresarCVV.existeTxtCVV()){
						...
						Thread.sleep(1500);
						//EvidenceData.capturarPaso(true, "Correcto: Se ingresan correctamente valores de seguridad ", driver);
						...
						if(VentanaActivacion.existeLblMensajeEnvioSMS()){
							if(VentanaActivacion.existeUrlNoRecibiMensaje()){
								...
								if(VentanaActivacion.existeLblModalAviso()){
									VentanaActivacion.clickBtnModalNo();
