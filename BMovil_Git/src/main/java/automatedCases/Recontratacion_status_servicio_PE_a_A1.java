...
public class Recontratacion_status_servicio_PE_a_A1 {
		...
		try 
		{
			...
			if(VentanaPrincipal.existeLblHola()){
				...
				if (IngresarTarjeta.existeTxtNumTarjeta()) {
					...
					if (LogResultInstrument1.getEstatusServicio().equals("PE")) {
						...
						CommonFunction.evidenciaError(false,"Error: El estatus de servicio no es PE, es "+LogResultInstrument1.getEstatusServicio());
					}
				} else {
