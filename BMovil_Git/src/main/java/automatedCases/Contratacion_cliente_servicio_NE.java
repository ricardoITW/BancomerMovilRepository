...
public class Contratacion_cliente_servicio_NE {
		...
		try
		{
			...
			if (VentanaPrincipal.existeLblHola()) {
				...
				if (IngresarTarjeta.existeTxtNumTarjeta()) {
					...
					if (LogResultInstrument1.getEstatusServicio().equals("NE")) {
						EvidenceData.capturarPaso(false, "Correcto: El estatus del servicio es NE", driver);
									...
									if (VentanaFirma.ExisteAreaParaFirma()) {
										...
										if (PantallaTerminosyCondiciones.existeLblTituloTerminosyCondiciones()) {
											...
											if (VentanaActivacionExitosa.existeImgPalomita()) {
												...
												if (LogResultInstrument2.getEstatusServicio().equals("PA")) {
													...
													CommonFunction.evidenciaError(true,"Error: El estatus de servicio no es PA, es "+LogResultInstrument2.getEstatusServicio());
												}
											} else {
						...
						CommonFunction.evidenciaError(true,"El estatus del servicio no es NE, es "+LogResultInstrument1.getEstatusServicio());
					}
